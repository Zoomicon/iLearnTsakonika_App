...
java -jar bundletool.jar build-apks --bundle=Release\iLearnTsakonika_App\bin\iLearnTsakonika_App.aab --output=iLearnTsakonika_App.apks --overwrite --mode=universal --ks=%UserProfile%\OneDrive\Zoomicon\Android\Keystore.keystore --ks-pass=pass:%PWD_KEYSTORE% --ks-key-alias=readcom --key-pass=pass:%PWD_KEY%
